...
async function handler(event) {
  ...
  const email = String(d2.email || "").trim().toLowerCase();
  const jobId = String(d2.jobId || d2.job_id || "");
  ...
  await db`insert into applications (id, job_id, name, email, phone, cover_note, cv_text, cv_filename, cv_data)
    values (${randomUUID()}::uuid, ${jobId}::uuid, ${name}, ${email}, ${String(d2.phone || "")}, ${String(d2.coverNote || d2.cover_note || "")},
      ${String(d2.cvText || d2.cv_text || "")}, ${d2.cvFilename || null}, ${d2.cvData || null})`;
