...
function readToken(event) {
  ...
  const m2 = cookie.match(/(?:^|;\s*)hf_session=([^;]+)/);
  const raw = m2 ? decodeURIComponent(m2[1]) : (event.headers.authorization || "").replace(/^Bearer\s+/i, "");
...
async function handler(event) {
  ...
  const d2 = parseBody(event);
  const title = String(d2.title || "").trim();
  const company = String(d2.company || "").trim();
  const location = String(d2.location || "").trim();
  const description = String(d2.description || "").trim();
  ...
  await db`insert into jobs (id, title, company, location, type, salary, category, description, status, flag_reasons)
    values (${id}::uuid, ${title}, ${company}, ${location}, ${String(d2.type || "Full-time")}, ${String(d2.salary || "")},
      ${String(d2.category || "Other")}, ${description}, ${status}, ${flag})`;
