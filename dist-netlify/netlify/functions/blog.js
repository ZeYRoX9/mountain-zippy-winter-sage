...
async function handler(event) {
  ...
  if (event.httpMethod === "GET") {
    const slug2 = event.queryStringParameters?.slug;
    if (slug2) {
      const rows2 = await db`select * from blog_posts where slug = ${slug2} and published = true limit 1`;
      return json(200, { post: rows2[0] || null });
    }
