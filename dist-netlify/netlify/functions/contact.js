...
async function handler(event) {
  ...
  const email = String(d2.email || "").trim();
  const message = String(d2.message || "").trim();
  ...
  await db`insert into messages (id, name, email, topic, message)
    values (${randomUUID()}::uuid, ${name}, ${email}, ${String(d2.topic || "general")}, ${message})`;
  const owner = process.env.OWNER_EMAIL || "support@hiredfrex.com";
  await sendResend({
    to: owner,
    subject: `HiredFrex contact: ${d2.topic || "general"}`,
